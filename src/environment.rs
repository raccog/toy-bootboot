...
use uefi::{
    CString16,
    prelude::*,
    proto::media::file::{Directory,FileMode,FileAttribute,File,RegularFile},
};
...
pub fn get_environment(image_handle: Handle, st: &mut SystemTable<Boot>, env: &mut [u8; 4096]) -> Result<usize, usize> {
    // Boot table
    ...
    let dir = root.open(&dirname, FileMode::Read, FileAttribute::DIRECTORY)
    ...
    let file = dir.open(&filename, FileMode::Read, FileAttribute::empty())
    ...
    let size = file.read(&mut env[..])
        .expect("Could not read BOOTBOOT/CONFIG file");
    file.close();

    // Ensure size of environment is less than a page
    if size > 4096 {
        error!("Size of environment ({} bytes) is larger than a page of memory (4096 bytes)", size);
        return Err(size);
    }
